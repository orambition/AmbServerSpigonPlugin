            ...
            if (strings[0].equals("permission") && strings.length > 4) {
                ...
                if (strings[1].equals("-u")) {
                    try {
                        ...
                        commandSender.sendMessage("UUID" + strings[3] + "错误");
                        return false;
                    }
                } else {
                    Player tPlayer = Bukkit.getServer().getPlayer(strings[3]);
                    if (tPlayer == null) {
                        commandSender.sendMessage("玩家" + strings[3] + "不在线,请使用-u进行uuid操作");
                        return false;
                    }
                ...
                if (strings[2].equals("add")) {
                    ...
                    commandSender.sendMessage("添加权限成功!" + uuid + "+" + strings[4]);
                } else if (strings[2].equals("del")) {
                    PermissionService.delPermission(uuid, strings[4]);
                    commandSender.sendMessage("删除权限成功!" + uuid + "-" + strings[4]);
                } else if (strings[2].equals("get")) {
                    List<String> pps = PermissionService.getPermission(uuid);
                    commandSender.sendMessage((pps == null || pps.isEmpty()) ? "结果为空" : pps.toString());
                }
                return true;
            }
        }
